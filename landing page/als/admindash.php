...
// Function to delete a band by ID
function deleteBand($conn, $bandId) {
    $bandId = $conn->real_escape_string($bandId);
    $sql = "DELETE FROM bands WHERE id = $bandId";
    return $conn->query($sql);
}

// Fetch booking reports
function getBookingReports($conn) {
    $sql = "SELECT * FROM booking_requests";
    $result = $conn->query($sql);
    return $result->fetch_all(MYSQLI_ASSOC);
}

$bookingReports = array(); // Initialize an empty array
...
// Check if the form is submitted for band deletion
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['band_id'])) {
    $bandId = $_POST['band_id'];

    if (deleteBand($conn, $bandId)) {
        echo '<p>Band deleted successfully!</p>';
    } else {
        echo '<p>Error deleting band: ' . $conn->error . '</p>';
    }
}
